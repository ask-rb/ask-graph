...
Patch numbers advance by exactly one per release: `0.7.8` → `0.7.9` →
`0.7.10`. Never skip or jump patch numbers (`0.7.8` → `0.7.10` is wrong),
...
gemchain guard ask-graph
gemchain update ask-graph <new-version> --dry-run
gemchain update ask-graph <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-graph.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/graph/version.rb` by gemchain, not by hand.
...
| `lib/ask/graph/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.7.8` → `0.7.9`.
- Several bug fixes ship together: one release, `0.7.8` → `0.7.9` (never
  `0.7.10`).
- New backwards-compatible API: `0.7.8` → `0.8.0`.
- Breaking change while pre-1.0: `0.8.0` → `0.9.0`, with a `Changed` /
  `Removed` changelog entry calling out the break.
- First stable release: `0.9.x` → `1.0.0`.
- This gem bumped because a dependency released: patch only, e.g. `0.7.9` →
  `0.7.10`, via the gemchain cascade — never hand-published.
